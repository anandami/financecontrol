...
- **Cadeia de modelos com Gemma 4 (2026-09-24)**: o 503 "high demand" apareceu de novo nos **dois** Flash ao mesmo tempo, então o par principal/reserva virou uma cadeia (`GEMINI_MODELS` em `src/ai.js`): `gemini-flash-latest` → `gemini-3.5-flash` → `gemini-3.5-flash-lite` → `gemma-4-26b-a4b-it` → `gemma-4-31b-it`, mesma chave e mesma API, cada um com cota/capacidade isolada. Só o principal retenta 503; os de reserva são tentados uma vez cada (pior caso: 7 chamadas). Para os Gemma, structured output não é garantido na Gemini API, então vão sem `responseMimeType`/`responseSchema` e o parse passou a tolerar JSON em bloco markdown e parts de raciocínio (`thought`). Os Gemma 4 26B/31B não aceitam áudio: nota de voz para no flash-lite. Validado ao vivo com a chave real (texto, lote e foto em todos os modelos): Gemma responde certo mas leva 15-50s (o 31B chegou a travar 60s antes de um 503, por isso é o último), `responseSchema` quebra a resposta dele (`{}`) e `responseMimeType` deu HTTP 500 no 31B; em comprovante ele lança item por item em vez do total. Reenvios do Telegram por demora já são ignorados pelo dedupe de `update_id`.
- **Orçamento de tempo na cadeia (2026-09-24)**: o painel de Execuções da planilha pessoal mostrava `doPost` de 290-357s e um de 360s com "Tempo limite atingido" (limite de 6 min do Apps Script) — ~60s por chamada, ou seja, sob "high demand" o 503 demora a voltar. Com 7 chamadas no pior caso, a cadeia nova passaria do limite e o dono não receberia nem o aviso de erro. Agora: (1) 503 só é retentado no mesmo modelo se voltou rápido (<10s), senão passa direto pro próximo; (2) a cadeia tem orçamento de 5 min (`GEMINI_TIME_BUDGET_MS`) e só começa uma tentativa se couber mais uma chamada de pior caso (65s), sobrando tempo pra mandar o erro no Telegram. Com 503 lentos em todos, ainda chega até o Gemma 26B (4 chamadas). Logs de execução do Web App não aparecem no painel (sem projeto GCP vinculado).
- **Velocidade: modelos "lite" primeiro (2026-09-24)**: respostas normais levavam 40-55s (painel de Execuções) e até 5 min sob sobrecarga. Medição ao vivo com a chave real e os prompts do bot: `gemini-flash-latest` (hoje 3.8-flash) levava ~9s mesmo quando respondia (raciocínio) e dava 503 na maioria das chamadas, às vezes só depois de 50-63s; `gemini-3.5-flash` 7-16s; `gemini-3.5-flash-lite` e `gemini-3.1-flash-lite` 1-3s, com o mesmo resultado em texto, foto, extrato com 9 linhas (mesmas 7 despesas/categorias, ignorando PIX recebido/estorno/saldo), áudio e correção. Nova ordem: `3.5-flash-lite` → `3.1-flash-lite` → `3.5-flash` → `flash-latest` → Gemma 26B → Gemma 31B. Versão do lite fixada em vez do alias `gemini-flash-lite-latest` (mesmo modelo hoje), pra não repetir o problema do alias pular pra um modelo recém-lançado e sobrecarregado.
